...
pub fn extract_text(file_name: &str, bytes: &[u8], max_chars: usize) -> anyhow::Result<String> {
    ...
    let text = match extension.as_str() {
        ...
        "doc" => {
            anyhow::bail!("Legacy .doc files are not supported; convert the document to .docx")
        }
        _ => anyhow::bail!("Only .txt and .docx files are supported"),
    };
    if text.trim().is_empty() {
        anyhow::bail!("Document does not contain text");
    }
    if text.chars().count() > max_chars {
        anyhow::bail!("Document text exceeds the configured limit");
    }
    Ok(text)
}
...
fn extract_docx(bytes: &[u8]) -> anyhow::Result<String> {
    if !bytes.starts_with(b"PK") {
        anyhow::bail!("DOCX file does not have a ZIP signature");
    }
